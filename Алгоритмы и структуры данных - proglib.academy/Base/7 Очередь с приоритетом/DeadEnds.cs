...
/// Тупики.
/// </summary>
public sealed class DeadEnds
{
    public int Execute((int start, int end)[] values)
    {
        var queue = new PriorityQueue();
        var result = 0;
        foreach (var (start, end) in values)
        {
            // пока минимум в контейнере меньше, чем прибытие новой электрички, извлекаем
            // удалять всех у кого время отправления меньше, чем новое время прибытия
            ...
            // новое количество
            result = queue.Count >= result ? queue.Count : result;
        }

        return result;
    }

    // тут поменял только < и >, чтобы меньшие элементы были выше
    private sealed class PriorityQueue
    {
        ...
        private void SiftUp(int index)
        {
            while (index > 0)
            {
                ...
                if (!(items[parent] > items[index])) // было items[parent] < items[index]
                {
                    return;
                }
        ...
        private void SiftDown(int index)
        {
            while (true)
            {
                ...
                if (left < items.Count && items[index] > items[left]) // было left < items.Count && items[index] < items[left]
                {
                    largest = left;
                }
                if (right < items.Count && items[largest] > items[right]) // было right < items.Count && items[largest] < items[right]
                {
                    largest = right;
                }
                if (largest == index)
